...
const searchInput =
    document.getElementById("lesson-search");

const results =
    document.getElementById("lesson-results");
...
async function loadLessons() {
    ...
    for (const levelData of levels) {
        for (
            ...
            requests.push(loadLesson(lessonID));
        }
    }
...
function displayLessons(list) {
    ...
    for (const lesson of list) {
        ...
        lessonIDText.className = "lesson-id";
        ...
        openButton.href =
            `lesvie.html?id=${encodeURIComponent(
                lesson.lessonID
            )}`;

        card.appendChild(title);
        card.appendChild(lessonIDText);
